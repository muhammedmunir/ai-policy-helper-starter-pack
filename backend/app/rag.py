...
# ---- PDPA Compliance Masking ----
_IC_PATTERN = re.compile(r"\b\d{6}-\d{2}-\d{4}\b")
_EMAIL_PATTERN = re.compile(r"\b[A-Za-z0-9._%+\-]+@[A-Za-z0-9.\-]+\.[A-Za-z]{2,}\b")
_PHONE_PATTERN = re.compile(r"\b(?:\+?60|0)[\s\-]?\d[\d\s\-]{7,10}\b")

def mask_pii(text: str) -> str:
    """Mask Malaysian IC numbers, email addresses, and phone numbers per PDPA."""
    text = _IC_PATTERN.sub("[IC REDACTED]", text)
...
class OllamaLLM:
    def __init__(self, host: str = "http://ollama:11434", model: str = "llama3"):
    ...
    def __init__(self):
        ...
        if settings.llm_provider == "openrouter" and settings.openrouter_api_key:
            ...
            try:
                ...
                self.llm_name = f"ollama:{settings.llm_model}"
            except Exception:
                self.llm = StubLLM()
                self.llm_name = "stub"
        else:
